...
import { ChevronDown, FileCheck2, FileText, Undo2, Receipt, Loader2 } from "lucide-react"
...
import { ValoracionItem, deshacerValorizacion } from "@/lib/actions/ventas"
import { ValorizarVentaDialog } from "./valorizar-venta-dialog"
import { RegistrarFacturaVentaDialog } from "./registrar-factura-venta-dialog"
...
export function ValoracionesTable<TData, TValue>({
    columns,
    data,
    pageCount,
    ...
    const table = useReactTable({
        data,
        columns,
        pageCount, // Server-side pagination if needed, but here we do manual params
        state: {
            sorting,
            columnFilters,
            columnVisibility,
    ...
    // Basic Filter Handlers (URL based would be better for persistence, but local for now)
    // For production app, wire these inputs to URL params (router.push)
        ...
        <div className="w-full space-y-4">
            <div className="flex items-center justify-between py-4">
                <div className="flex flex-1 items-center space-x-2">
                    ...
                    {/* Simple State Filter Button Group */}
                ...
                <div className="flex items-center space-x-2">
                    {(() => {
                        ...
                        const selectedData = selectedRows.map((r) => r.original as ValoracionItem)
                        const allPendientes = selectedData.length > 0 && selectedData.every((d) => d.estado === 'PENDIENTE')
                        // Para acciones sobre valorizaciones existentes (PDF,
                        // Deshacer) exigimos que todas las filas seleccionadas
                        // pertenezcan a la MISMA valorización — un código comparte
                        // N filas de reportes, operar sobre código completo.
                        ...
                        return (
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end" className="w-64">
                                    ...
                                    <DropdownMenuSeparator />
                                    {/* PDF Valorización — una sola valorización */}
                                    <DropdownMenuItem
                                        disabled={!singleCodigo}
                                        onClick={() => {
                                            if (singleCodigo) {
                                                window.open(`/api/valorizaciones/${encodeURIComponent(singleCodigo)}/pdf`, '_blank', 'noopener,noreferrer')
                                            }
                                        }}
                                        className="gap-2"
                                    >
                                        ...
                                        PDF Valorización
                                        {!singleCodigo && selectedRows.length > 0 && (
                                            <span className="text-[10px] text-gray-400 ml-auto">1 valorización</span>
                                        )}
                                    </DropdownMenuItem>
                                    {/* Deshacer Valorización — una sola, todos VALORADO */}
                                    <DropdownMenuItem
                                        disabled={!singleCodigo || !allValorados}
                                        onClick={() => {
                                            if (singleCodigo) {
                                                setDeshacerCodigo(singleCodigo)
                                    ...
                                    <DropdownMenuItem
                                        ...
                                        onClick={() => {
                                            if (singleCodigo) {
                                                setFacturarCodigo(singleCodigo)
            ...
            <ValorizarVentaDialog
                open={valorizarOpen}
                onOpenChange={setValorizarOpen}
                reporteIds={table
                    .getFilteredSelectedRowModel()
                    .rows.map((r) => (r.original as ValoracionItem).id)
                    .filter(Boolean)}
                onDone={() => {
            ...
            {/* Confirmación deshacer valorización */}
                    ...
                    <AlertDialogFooter>
                        ...
                        <AlertDialogAction
                            ...
                            onClick={async (e) => {
                                ...
                                const res = await deshacerValorizacion(deshacerCodigo)
                        ...
                        >
                            {deshacerPending ? (
                                <><Loader2 className="h-4 w-4 animate-spin mr-2" />Deshaciendo…</>
                            ) : (
                                'Deshacer valorización'
                            )}
                        </AlertDialogAction>
