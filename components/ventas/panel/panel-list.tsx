...
import { ClientGroupMetric } from "@/lib/actions/ventas";
...
export function PanelList({ title, data, page, total, limit, type }: PanelListProps) {
    ...
    const handleRowClick = (clienteId: string) => {
        ...
        router.push(`/ventas/valoraciones?cliente_id=${clienteId}&estado=${estado}`);
    };
                    ...
                    {data.length === 0 ? (
                        ...
                        data.map((item, index) => (
                            <TableRow
                                ...
                                onClick={() => handleRowClick(item.cliente_id)}
                            >
                                <TableCell className="text-center">{startItem + index}</TableCell>
                                <TableCell className="font-medium max-w-[200px] truncate" title={item.cliente_nombre}>
                                    {item.cliente_nombre}
                                </TableCell>
