...
import { FileText, Search, X, Eye, Pencil, ExternalLink } from "lucide-react"
...
import type { FacturaVentaItem } from "@/lib/actions/ventas"
import { FacturaVentaDialog } from "./factura-venta-dialog"

interface Props {
    data: FacturaVentaItem[]
    total: number
    kpis: { total_facturado_usd: number; total_cobrado_usd: number; total_pendiente_usd: number }
}
...
export function FacturasVentaTable({ data, total, kpis }: Props) {
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <Card>
                    <CardContent className="pt-6">
                        <div className="text-xs text-gray-500 uppercase tracking-wide">Total cobrado</div>
                        <div className="text-2xl font-bold text-green-700 mt-1">{fmtMoney(kpis.total_cobrado_usd)}</div>
                    </CardContent>
                </Card>
                <Card>
                    <CardContent className="pt-6">
                        <div className="text-xs text-gray-500 uppercase tracking-wide">Pendiente de cobro</div>
            ...
            <div className="flex flex-wrap items-center gap-3">
                <div className="relative flex-1 max-w-sm">
                    ...
                    <Input
                        ...
                        onKeyDown={(e) => {
                            if (e.key === 'Enter') pushParam('search', searchInput || null)
                        }}
                        onBlur={() => {
                            if (searchInput !== (searchParams.get('search') ?? '')) pushParam('search', searchInput || null)
                        }}
                ...
                <Table>
                    <TableHeader className="bg-gray-50/60">
                        <TableRow>
                            ...
                            <TableHead className="text-xs text-right">Detracción</TableHead>
                        ...
                        {data.length === 0 ? (
                            ...
                            data.map((f) => (
                                <TableRow key={f.id} className="hover:bg-gray-50/40">
                                    ...
                                    <TableCell className="text-xs font-medium max-w-[180px] truncate" title={f.cliente_nombre ?? ''}>
                                        {f.cliente_nombre ?? '—'}
                                    </TableCell>
                                    <TableCell className="text-xs font-mono">{f.cliente_ruc ?? '—'}</TableCell>
                                    ...
                                    <TableCell className="text-xs text-right">{fmtMoney(f.detraccion_monto_sol, 'PEN')}</TableCell>
                                    <TableCell className="text-xs text-right text-green-700">{fmtMoney(f.monto_pagado_factura)}</TableCell>
                                    ...
                                    <TableCell>
                                        <div className="flex items-center gap-0.5">
                                            <Button
                                                ...
                                                size="icon"
                                                className="h-7 w-7"
                                                onClick={() => { setDetailId(f.id); setEditMode(false) }}
                                                title="Ver"
                                            >
                                            ...
                                            <Button
                                                ...
                                                size="icon"
                                                className="h-7 w-7"
                                                onClick={() => { setDetailId(f.id); setEditMode(true) }}
                                                title="Editar"
                                            >
                                                <Pencil className="h-3.5 w-3.5" />
                                            </Button>
                                            {f.pdf_factura_url ? (
                                                <a href={f.pdf_factura_url} target="_blank" rel="noreferrer" title="Ver PDF cliente">
                                                    <Button variant="ghost" size="icon" className="h-7 w-7 text-blue-600">
                                                        <FileText className="h-3.5 w-3.5" />
                                                    </Button>
                                                </a>
                                            ) : (
                                                <Button variant="ghost" size="icon" className="h-7 w-7 text-gray-300" disabled title="Sin PDF del cliente cargado">
                                                    <FileText className="h-3.5 w-3.5" />
                                                </Button>
                                            )}
                                            {f.codigo_valoracion && (
                                                <a
                                                    href={`/api/valorizaciones/${encodeURIComponent(f.codigo_valoracion)}/pdf`}
                                                    target="_blank"
                                                    rel="noreferrer"
                                                    title={`PDF Valorización ${f.codigo_valoracion}`}
                                                >
