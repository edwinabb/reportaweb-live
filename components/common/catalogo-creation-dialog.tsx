...
import { createCatalogo } from "@/lib/actions/catalogos"

interface CatalogoCreationDialogProps {
    categoria: string
    label: string
    onItemCreated?: (newItem: { id: string, nombre: string }) => void
}

export function CatalogoCreationDialog({ categoria, label, onItemCreated }: CatalogoCreationDialogProps) {
    ...
    async function handleSave() {
        ...
        setIsLoading(true)
        const res = await createCatalogo(categoria, value)
        setIsLoading(false)
                ...
                <div className="grid gap-4 py-4">
                    <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <Input
                            ...
                            placeholder={`Ej. ${label === 'Cargo' ? 'Supervisor' : 'Operaciones'}`}
                        />
                    </div>
                </div>
                <DialogFooter>
