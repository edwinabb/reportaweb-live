...
import {
    getValorizacionCompraPreview,
    valorizarReportesCompra,
    type ValorizacionCompraPreview,
...
export function ValorizarCompraDialog({ open, onOpenChange, reporteIds, onDone }: Props) {
    const router = useRouter()
    const [loading, setLoading] = useState(false)
    const [submitting, setSubmitting] = useState(false)
    const [preview, setPreview] = useState<ValorizacionCompraPreview | null>(null)
    const [fecha, setFecha] = useState<string>(() => new Date().toISOString().slice(0, 10))

    useEffect(() => {
        if (!open || reporteIds.length === 0) { setPreview(null); return }
        setLoading(true)
        getValorizacionCompraPreview(reporteIds)
            .then(setPreview)
    ...
    const handleSubmit = async () => {
        ...
        setSubmitting(true)
        const res = await valorizarReportesCompra(reporteIds, fecha)
        setSubmitting(false)
            ...
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2">
                        ...
                        Valorizar Compra (Proveedor)
                    </DialogTitle>
                    <DialogDescription>
                        Agrupa {reporteIds.length} reporte(s) en una valorización de compra con código consecutivo.
                    </DialogDescription>
                </DialogHeader>
                ...
                {loading ? (
                    ...
                    <div className="text-center py-8 text-sm text-gray-500">No se encontraron reportes válidos.</div>
                ) : (
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            ...
                            <div className="space-y-1">
                                ...
                                <div className="text-sm font-medium truncate" title={preview.proveedor_nombre ?? ''}>
                            ...
                            <div className="space-y-1">
                                ...
                                <Input id="fecha" type="date" value={fecha} onChange={(e) => setFecha(e.target.value)} className="h-8" />
                            </div>
                        </div>
                        ...
                        <div className="flex justify-end">
                            <div className="w-full max-w-xs space-y-1 text-sm">
                                ...
                                <div className="flex justify-between border-t pt-1 mt-1 font-bold">
                                    ...
                                    <span className="text-orange-600">{fmt(preview.total_pagar, preview.moneda)}</span>
                                </div>
                            </div>
                        </div>
                    </div>
