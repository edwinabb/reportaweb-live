...
import {
    createFacturaCompra,
    getValoracionesComprasData,
    type ValoracionCompraItem,
...
export function RegistrarFacturaCompraDialog({ open, onOpenChange, codigoValoracion, onDone }: Props) {
    const router = useRouter()
    const [loading, setLoading] = useState(false)
    const [submitting, setSubmitting] = useState(false)
    const [uploading, setUploading] = useState(false)
    const [items, setItems] = useState<ValoracionCompraItem[]>([])
    ...
    useEffect(() => {
        ...
        setLoading(true)
        // Traer items de esa valorización
        getValoracionesComprasData({ estado: 'VALORADO', limit: 1000 })
            .then((res) => {
    ...
    const proveedor = items[0]?.proveedor_nombre || '—'
    const moneda = (items[0]?.moneda || 'USD') as 'USD' | 'PEN'
    const subtotal = items.reduce((s, it) => s + it.total_compra, 0)
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            fd.append('file', file)
            fd.append('bucket', 'cotizaciones') // reusamos bucket existente
            fd.append('subfolder', 'facturas-compra')
    ...
    const handleSubmit = async () => {
        ...
        if (!codigoFactura.trim()) { toast.error('El N° de factura del proveedor es obligatorio'); return }
        if (!pdfUrl) { toast.error('Subí el PDF de la factura del proveedor'); return }
        if (!fechaFactura) { toast.error('La fecha de factura es obligatoria'); return }
        const proveedorId = items[0]?.proveedor_id
        if (!proveedorId) { toast.error('La valorización no tiene proveedor asociado'); return }
        ...
        const res = await createFacturaCompra({
            ...
            detraccion_soles: moneda === 'PEN' ? detrMonto : null,
            detraccion_usd: moneda === 'USD' ? detrMonto : null,
            pdf_factura_url: pdfUrl,
        })
        setSubmitting(false)
        if (res.success) {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Registra la factura recibida del proveedor a partir de la valorización <b>{codigoValoracion}</b>.
                        El PDF recibido del proveedor es obligatorio.
                    </DialogDescription>
                </DialogHeader>
                ...
                {loading ? (
                    ...
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 bg-gray-50 p-3 rounded-md">
                            <div><Label className="text-xs text-gray-500">Proveedor</Label><div className="font-semibold text-sm">{proveedor}</div></div>
                            ...
                            <div><Label className="text-xs text-gray-500">Cant. facturar total</Label><div className="text-sm">{items.reduce((s, i) => s + i.cantidad_facturar, 0).toFixed(2)}</div></div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                            <div className="space-y-1">
                                <Label htmlFor="codigo_factura">N° Factura Proveedor *</Label>
                            ...
                            <div className="space-y-1">
                                <Label htmlFor="dias_pago">Días para Pago</Label>
                            ...
                            <div className="space-y-1">
                                <Label htmlFor="detr_pct">% Detracción (retenida)</Label>
                                <Input id="detr_pct" type="number" value={detrPct} onChange={(e) => setDetrPct(e.target.value)} placeholder="Ej: 10" />
                            </div>
                        </div>

                        <div className="border rounded-md p-3">
                            <Label className="text-xs text-gray-500 mb-2 block">PDF Factura recibida del Proveedor *</Label>
                            {pdfUrl ? (
                ...
                <DialogFooter>
                    ...
                    <Button
                        ...
                        disabled={submitting || loading || items.length === 0 || !pdfUrl || !codigoFactura.trim()}
